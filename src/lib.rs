...
//!
//! * `#[builder(generate = "false")]` causes setter methods to not be generated by default.
//! * `#[builder(generate_private = "false")]` causes setter methods to not be generated by
//!   default for private fields.
//! * `#[builder(generate_public = "false")]` causes setter methods to not be generated by default
//!   for public fields.
//! * `#[builder(no_std)]` causes the generated code to use `core` instead of `std`.
//! * `#[builder(prefix = "with_")]` causes the setter method on all fields to be prefixed with
//!   the given string.
//!
//! The following options can be set on a fields.
//!
//! * `#[builder(generate)]` causes a setter method to be generated, overriding struct-level
//!   settings.
//! * `#[builder(skip)]` causes no setter method to be generated.
//! * `#[builder(rename = "setter_name")]` causes the setter method to have a different name from
//!   the field. This overwrites `add_prefix`.
//!
//! The following options can be set on either the entire struct, or on an individual field. You
//! can disable the features for a particular field with `#[builder(option = "false")]`
//!
//! * `#[builder(into)]` causes the setter method to accept any type that can be converted into
//!   the field's type via [`Into`].
//! * `#[builder(strip_option)]` causes the setter method to accept `T` instead of `Option<T>`. If
