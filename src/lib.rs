...
//! * `#[setters(generate_public = "false")]` causes setter methods to not be generated by default
//!   for public fields.
//! * `#[setters(no_std)]` causes the generated code to use `core` instead of `std`.
//! * `#[setters(prefix = "with_")]` causes the setter method on all fields to be prefixed with
//!   the given string.
//!
...
//! * `#[setters(rename = "setter_name")]` causes the setter method to have a different name from
...
//!
//! * `#[setters(into)]` causes the setter method to accept any type that can be converted into
